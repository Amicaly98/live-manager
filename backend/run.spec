...
# - 需要内置初始分区时，用环境变量指向"仓库外的明确文件"（例如
#   deliveries/area-git-exit-20260915/area-backup/desktop/root-bili_areas_full.json
#   或 backend/build-inputs/bili_areas_full.json），本脚本会校验结构后再打进去；
...
def _load_and_validate_seed(path: Path):
    ...
    只接受"非空 list、元素含整数 id 与非空 name"的结构，避免把破损/空表
    打进安装包（装上去别人看到的就是"有种子但加载失败"这种假象）。
    """
    if not path.exists():
    ...
    if not isinstance(data, list) or not data:
        return "种子内容必须是非空数组", None
    for item in data:
        if not isinstance(item, dict) or not str(item.get("name", "")).strip():
            return "种子元素必须是含 name 的对象", None
        try:
            int(item.get("id"))
        except (TypeError, ValueError):
            return f"种子元素 id 不是整数：{item!r}"[:200], None
    return "", data
...
def _resolve_seed():
    ...
    if not seed_env:
        ...
        os.environ.setdefault(PROVENANCE_ENV, "")
        return []
    ...
    line = ("[area-seed-provenance] source=%s sha256=%s source_mtime=%s "
            "built_utc=%s toplevel=%d note=构建输入（非受跟踪文件）"
