...
test('E1: classifyPortConflict 区分自有/未知/空闲', () => {
  assert.equal(classifyPortConflict([], [1]), 'none');
  assert.equal(classifyPortConflict([7, 8], [7, 8]), 'own_stale');
  assert.equal(classifyPortConflict([7, 9], [7]), 'foreign');
  assert.equal(classifyPortConflict([9], []), 'foreign');
});
